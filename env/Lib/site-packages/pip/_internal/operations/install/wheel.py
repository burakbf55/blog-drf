...
def _normalized_outrows(outrows):
    ...
    return sorted(
        (ensure_str(record_path, encoding="utf-8"), hash_, str(size)) for record_path, hash_, size in outrows
    )
...
def get_console_script_specs(console):
    ...
    if pip_script:
        ...
        scripts_to_generate.append("pip{} = {}".format(get_major_minor_version(), pip_script))
