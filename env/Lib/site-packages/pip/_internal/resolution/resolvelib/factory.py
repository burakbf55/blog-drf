...
class Factory(object):
    def __init__(
        ...
        if not ignore_installed:
            self._installed_dists = {
                canonicalize_name(dist.project_name): dist for dist in get_installed_distributions()
            }
        else:
            self._installed_dists = {}
    ...
    def find_candidates(self, requirements, constraint):
        ...
        return (c for c in explicit_candidates if all(req.is_satisfied_by(c) for req in requirements))
    ...
    def _report_requires_python_error(
        ...
        message_format = "Package {package!r} requires a different Python: " "{version} not in {specifier!r}"
    ...
    def get_installation_error(self, e):
        ...
        if len(e.causes) == 1:
            ...
            return DistributionNotFound("No matching distribution found for {}".format(req))
