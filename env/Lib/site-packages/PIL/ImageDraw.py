...
class ImageDraw:
    ...
    def rounded_rectangle(self, xy, radius=0, fill=None, outline=None, width=1):
        ...
        if ink is not None and ink != fill and width != 0:
            ...
            if not full_x:
                self.draw.draw_rectangle((x0 + r + 1, y0, x1 - r - 1, y0 + width - 1), ink, 1)
                self.draw.draw_rectangle((x0 + r + 1, y1 - width + 1, x1 - r - 1, y1), ink, 1)
            if not full_y:
                self.draw.draw_rectangle((x0, y0 + r + 1, x0 + width - 1, y1 - r - 1), ink, 1)
                self.draw.draw_rectangle((x1 - width + 1, y0 + r + 1, x1, y1 - r - 1), ink, 1)
    ...
    def multiline_text(
        ...
        line_spacing = self.textsize("A", font=font, stroke_width=stroke_width)[1] + spacing
        for line in lines:
    ...
    def textsize(
        ...
        if self._multiline_check(text):
            return self.multiline_textsize(text, font, spacing, direction, features, language, stroke_width)
    ...
    def multiline_textsize(
        ...
        line_spacing = self.textsize("A", font=font, stroke_width=stroke_width)[1] + spacing
        for line in lines:
            line_width, line_height = self.textsize(line, font, spacing, direction, features, language, stroke_width)
            max_width = max(max_width, line_width)
    ...
    def textlength(
        ...
        try:
            ...
            size = self.textsize(text, font, direction=direction, features=features, language=language)
            if direction == "ttb":
                return size[1]
            return size[0]
...
def _compute_regular_polygon_vertices(bounding_circle, n_sides, rotation):
    ...
    if len(bounding_circle) == 3:
        *centroid, polygon_radius = bounding_circle
        ...
        raise ValueError(
            "bounding_circle should contain 2D coordinates " "and a radius (e.g. (x, y, r) or ((x, y), r) )"
        )
    ...
    if not len(centroid) == 2:
        raise ValueError("bounding_circle centre should contain 2D coordinates (e.g. (x, y))")
