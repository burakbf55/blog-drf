...
def createProfile(colorSpace, colorTemp=-1):
    ...
    if colorSpace not in ["LAB", "XYZ", "sRGB"]:
        raise PyCMSError(f"Color space not supported for on-the-fly profile creation ({colorSpace})")

    if colorSpace == "LAB":
        try:
            colorTemp = float(colorTemp)
        except (TypeError, ValueError) as e:
            raise PyCMSError(f'Color temperature must be numeric, "{colorTemp}" not valid') from e
