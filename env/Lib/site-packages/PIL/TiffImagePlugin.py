...
class ImageFileDirectory_v2(MutableMapping):
    ...
    def _register_basic(idx_fmt_name):
        ...
        _load_dispatch[idx] = (  # noqa: F821
            size,
            lambda self, data, legacy_api=True: (self._unpack("{}{}".format(len(data) // size, fmt), data)),
        )
    ...
    def _ensure_read(self, fp, size):
        ret = fp.read(size)
        if len(ret) != size:
            raise OSError("Corrupt EXIF data.  " f"Expecting to read {size} bytes but only got {len(ret)}. ")
        return ret
