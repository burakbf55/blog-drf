...
async def black_run(
    ...
    timeout = project_config["timeout_seconds"] if "timeout_seconds" in project_config else TEN_MINUTES_SECONDS
    with TemporaryDirectory() as tmp_path:
        ...
        try:
            ...
            _stdout, _stderr = await _gen_check_output(cmd, cwd=cwd_path, env=env, stdin=stdin, timeout=timeout)
        except asyncio.TimeoutError:
            results.stats["failed"] += 1
...
def handle_PermissionError(func: Callable[..., None], path: Path, exc: Tuple[Any, Any, Any]) -> None:
    """
